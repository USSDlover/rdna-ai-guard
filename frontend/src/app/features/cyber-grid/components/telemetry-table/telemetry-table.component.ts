...
export class TelemetryTableComponent {
  ...
  // Modal state
  protected selectedEvent = signal<TelemetryEvent | null>(null);

  protected openModal(event: TelemetryEvent): void {
  ...
  protected riskScoreClasses(score: number): string {
    if (score >= 75) return 'text-red-400 font-semibold';
    if (score >= 40) return 'text-amber-300 font-medium';
